...
//   • keep the motion via Animated transforms (native driver)
//   • read the palette from `theme.aurora`, so switching palette in Settings
...
function makeBlobs(accent: { blue: string; violet: string; cyan: string; pink: string }): BlobDef[] {
  ...
  return [
    // blob-a — top-left
    { color: accent.blue, dx: 60, dy: 120, scaleTo: 1.12, duration: 18000, left: -120, top: -160, size: S },
    // blob-b — top-right
    { color: accent.violet, dx: -80, dy: 80, scaleTo: 1.08, duration: 22000, left: SCREEN_W - S + 180, top: -80, size: S },
    // blob-c — bottom-left
    { color: accent.cyan, dx: 100, dy: -80, scaleTo: 0.95, duration: 26000, left: -100, top: SCREEN_H - S + 200, size: S },
    // blob-d — bottom-right
    { color: accent.pink, dx: -60, dy: -100, scaleTo: 1.1, duration: 20000, left: SCREEN_W - S + 120, top: SCREEN_H - S + 140, size: S },
  ];
}
...
function Blob({ def, idx }: { def: BlobDef; idx: number }) {
  ...
  React.useEffect(() => {
    const loop = Animated.loop(
      Animated.sequence([
        Animated.timing(anim, { toValue: 1, duration: def.duration, easing: Easing.inOut(Easing.sin), useNativeDriver: true }),
        Animated.timing(anim, { toValue: 0, duration: def.duration, easing: Easing.inOut(Easing.sin), useNativeDriver: true }),
      ]),
    );
    loop.start();
    return () => loop.stop();
  ...
  return (
    <Animated.View
      ...
      style={{
        ...
        width: def.size,
        height: def.size,
        // `mix-blend-mode: screen` isn't available on RN Views; high opacity over
        // a dark backdrop with overlapping light colors approximates the look.
        opacity: 0.65,
...
export function AppBackground() {
  ...
  return (
    <View style={StyleSheet.absoluteFill} pointerEvents="none">
      ...
      {/* Top shimmer + bottom fade, matches prototype's overlays */}
      <LinearGradient
