// src/theme.ts — Aurora design system (dark only).
//
// One aesthetic, no light mode. Personalization comes from two knobs:
//   palette          aurora / violet / emerald / sunset
//   glassIntensity   0..100 slider (default 65)
//
// The glass look is driven by `glassIntensity` via the exact formula from the
// Gymdash.html prototype's Glass component (see `glassTokensFor()`). Buttons,
// chips and inputs all inherit those same numbers through `theme.glass` /
// `theme.glassBorder`, so the whole app reads as one frosted surface.
//
// Back-compat: `ThemeMode` / `setThemeMode` / `getThemeMode` are kept as
...
/** @deprecated dark-only now — retained so existing imports keep compiling. */
export type ThemeMode = "light" | "dark" | "system";

export type Theme = {
  /** Always "dark" now. */
  mode: "dark";
  /** Always true now; kept so `theme.isDark ? a : b` call sites still work. */
  isDark: boolean;
  ...
  /** Secondary muted ink. */
  muted2: string;
  /** Tertiary ink (labels that need to recede further). */
  ink3: string;
  line: string;
  divider: string;

  // Glass — raw intensity-derived numbers + composed colors
...
// ── Aurora palettes (the signature accents behind the glass) ─────────────────
...
// ── Glass intensity (0..100) — mirrors the prototype's Glass component ────────
//   blur    = 8  + k*24    // 8..32
//   sat     = 120 + k*60   // 120..180
//   fillA   = 0.04 + k*0.10 // 0.04..0.14
//   fillB   = 0.02 + k*0.06 // 0.02..0.08
//   strokeA = 0.12 + k*0.22 // 0.12..0.34
...
function createTheme(palette: Palette, intensity: number): Theme {
  ...
  // Bump the strong variant ~1.6x to match the prototype's `strong` feel.
...
/** Accepts any 0..100 number; clamped + rounded on set. */
...
/** @deprecated dark-only now — no-op kept so existing call sites compile. */
export function setThemeMode(_mode?: ThemeMode) {
  /* intentionally empty: theme is dark-only */
}

/** @deprecated dark-only now — always reports "dark". */
